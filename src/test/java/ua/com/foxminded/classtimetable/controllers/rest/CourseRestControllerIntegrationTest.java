...
public class CourseRestControllerIntegrationTest {
    ...
    public void should_createNewCourse_when_controllerCallsAddToDBMethod() {
        ...
        assertThat(courseRestController.showAll(), not(hasItem(course)));

        courseRestController.addToDB(course);

        assertThat(courseRestController.showAll(), hasItem(course));
        assertEquals(courseRestController.showById(course.getId()), course);
    ...
    public void should_updateCourse_when_controllerCallsUpdateMethod() {
        ...
        Course course = courseRestController.showById(3);
        course.setCourseName("Name");

        courseRestController.update(course);

        assertEquals(courseRestController.showById(3).getCourseName(), "Name");
