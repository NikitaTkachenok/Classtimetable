...
public class LessonDao implements DaoInterface<Lesson> {
	...
	private static final String ADD_NEW_LESSON = "INSERT INTO lessons (start_time, end_time, classroom_id, course_id, teacher_id) VALUES (?, ?, ?, ?, ?)";
	private static final String UPDATE_LESSON = "UPDATE lessons SET start_time = ?, end_time = ?, classroom_id = ?, course_id = ?, teacher_id = ? WHERE id = ?";
	...
	public void update(Lesson lesson) {
		jdbcTemplate.update(UPDATE_LESSON, lesson.getStartTime(), lesson.getEndTime(), lesson.getClassroomId(),
				lesson.getCourseId(), lesson.getTeacherId(), lesson.getId());
	}
